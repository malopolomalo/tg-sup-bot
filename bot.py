...
async def start(message: types.Message):
    await message.answer(
        ...
        "- кнопкой 'Ответить' под сообщением\n"
        "- командой /send 123456789 текст"
    )
...
async def handle_user(message: types.Message):
    ...
    if message.text:
        await bot.send_message(ADMIN_ID, user_text + message.text, reply_markup=admin_keyboard(user.id))
    elif message.photo:
        await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=user_text, reply_markup=admin_keyboard(user.id))
    elif message.video:
        await bot.send_video(ADMIN_ID, message.video.file_id, caption=user_text, reply_markup=admin_keyboard(user.id))
    elif message.document:
        await bot.send_document(ADMIN_ID, message.document.file_id, caption=user_text, reply_markup=admin_keyboard(user.id))
    elif message.voice:
        await bot.send_voice(ADMIN_ID, message.voice.file_id, caption=user_text, reply_markup=admin_keyboard(user.id))
    
    await bot.send_message(
        ADMIN_ID,
        f"💡 Чтобы ответить:\n- Нажми 'Ответить'\n- Или /send {user.id} текст"
    )
...
async def handle_admin_message(message: types.Message):
    ...
    if not user_id:
        # Если нет ожидающего ответа — проверяем, может это команда /send
